...
// Query kuota
$kuota_query = "SELECT p.id, p.nama_poli, k.kuota, k.tanggal,
               (SELECT COUNT(*) FROM pendaftaran pd 
                WHERE pd.id_poli = p.id 
                AND pd.tanggal_periksa = k.tanggal) as terdaftar
               FROM poli p 
               LEFT JOIN kuota_poli k ON p.id = k.id_poli AND k.tanggal = '$tanggal_filter'
               WHERE 1=1";

if (!empty($poli_filter)) {
    $kuota_query .= " AND p.id = $poli_filter";
}

$kuota_query .= " ORDER BY p.nama_poli";
...
<head>
    ...
    <title>Cek Kuota - Puskesmas Nalumsari</title>
    ...
    <style>
        ...
        * {
        ...
        .badge-penuh { background: #fee2e2; color: #b91c1c; }
...
<body>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item">
                <a href="check_kuota.php" class="nav-link active">
    ...
    <main class="main-content">
        <header class="top-header">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <h2 class="page-title">Cek Kuota Poli</h2>
            </div>
        ...
        <div class="content-wrapper">
            ...
            <div class="card-box">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-search"></i>
                        Cek Ketersediaan Kuota Poli
                    </h3>
                </div>
                
                <!-- Filter Form -->
                <form method="GET" action="check_kuota.php" class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="tanggal" name="tanggal" 
                               class="form-control" value="<?php echo $tanggal_filter; ?>" 
                    ...
                    <div class="form-actions">
                        ...
                        <a href="check_kuota.php" class="btn btn-secondary">
            ...
            <div class="card-box">
                <div class="card-header">
                    ...
                    <div class="text-muted">
                        Tanggal: <?php echo date('d/m/Y', strtotime($tanggal_filter)); ?>
                    </div>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            if (mysqli_num_rows($kuota_result) > 0) {
                                mysqli_data_seek($kuota_result, 0);
                                while ($kuota = mysqli_fetch_assoc($kuota_result)) {
                                    $kuota_tersedia = $kuota['kuota'] ? $kuota['kuota'] : 10; // Default 10
                                    $terdaftar = $kuota['terdaftar'] ? $kuota['terdaftar'] : 0;
                                    $sisa_kuota = $kuota_tersedia - $terdaftar;
                                    
                                    // Tentukan status
                                    if ($sisa_kuota <= 0) {
                                        $status = 'Penuh';
                                        $status_class = 'badge-penuh';
                                    } elseif ($sisa_kuota <= 3) {
                                        $status = 'Terbatas';
                                ...
                                echo "<tr>
                                        <td colspan='6' class='no-data'>
                                            ...
                                            <div>Tidak ada data kuota untuk filter yang dipilih</div>
                                        </td>
                                      </tr>";
                            }
                            ?>
    ...
    <script>
        ...
        // Set minimum date for tanggal filter to today
